...
module.exports = async (clips, m, chatUpdate, plugins, store) => {
   try {
      ...
      // stories reaction
      clips.storyJid = clips.storyJid ? clips.storyJid  : []
      if (m.chat.endsWith('broadcast') && !clips.storyJid.includes(m.sender) && m.sender != clips.decodeJid(clips.user.id)) clips.storyJid.push(m.sender)
      if (m.chat.endsWith('broadcast') && [...new Set(clips.storyJid)].includes(m.sender) && !/protocol/.test(m.mtype)) {
         await clips.sendMessage('status@broadcast', {
            react: {
               text: Func.random(['🤣', '🥹', '😂', '😋', '😎', '🤓', '🤪', '🥳', '😠', '😱', '🤔']),
               key: m.key
            }
      ...
      if (((m.isGroup && !groupSet.mute) || !m.isGroup) && !users.banned) {
         if (body && body == isPrefix) {
            ...
            if (!banchat) {
               await clips.reply(m.chat, Func.texted('bold', `Checking . . .`), m)
               return clips.reply(m.chat, Func.texted('bold', `Response Speed: ${((new Date - old) * 1)}ms`), m)
            } else {
               await clips.reply(m.chat, Func.texted('bold', `Checking . . .`), m)
               return clips.reply(m.chat, Func.texted('bold', `Response Speed: ${((new Date - old) * 1)}ms (nonaktif)`), m)
            }
         }
      }
      ...
      if (body && isPrefix && commands.includes(command) || body && !isPrefix && commands.includes(command) && setting.noprefix || body && !isPrefix && commands.includes(command) && env.evaluate_chars.includes(command)) {
         ...
         for (let name in is_commands) {
            ...
            if (!m.isGroup && !['owner', 'menfess', 'verify'].includes(name) && chats && !isPrem && !users.banned && setting.groupmode) return clips.sendMessageModify(m.chat, `⚠️ Using bot in private chat only for premium user, upgrade to premium plan only Rp. 10,000,- to get 3K limits for 1 month.\n\nIf you want to buy contact *${prefixes[0]}owner*`, m, {
               largeThumb: true,
            ...
            if (m.isGroup && !['activation', 'groupinfo', 'exec', 'makeAdmin', 'mute', 'omute'].includes(name) && groupSet.mute) continue
            ...
            if (cmd.game && !setting.games) {
               clips.reply(m.chat, global.status.gameSystem, m)
               continue
            }
            if (cmd.game && Func.level(users.point, env.multiplier)[0] >= 50) {
               clips.reply(m.chat, global.status.gameLevel, m)
               continue
            }
            if (cmd.game && m.isGroup && !groupSet.game) {
               clips.reply(m.chat, global.status.gameInGroup, m)
               continue
            }
            cmd.async(m, {
               clips,
         ...
         for (let name in is_events) {
            ...
            if (!m.isGroup && env.blocks.some(no => m.sender.startsWith(no))) return clips.updateBlockStatus(m.sender, 'block')
            if (m.isGroup && !['exec', 'mute', 'omute'].includes(name) && groupSet.mute) continue
