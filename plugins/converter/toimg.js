...
exports.run = {
   ...
   async: async (m, {
      ...
      try {
         if (!m.quoted) return clips.reply(m.chat, Func.texted('bold', `{ SlrmyApi }  Reply to sticker you want to convert to an image/photo (not supported for sticker animation)`), m)
         if (m.quoted.mimetype != 'image/webp') return clips.reply(m.chat, Func.texted('bold', `{ SlrmyApi }  Reply to sticker you want to convert to an image/photo (not supported for sticker animation)`), m)
