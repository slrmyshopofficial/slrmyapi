exports.run = {
   ...
   async: async (m, {
      ...
      try {
         let exif = db.setting
         if (!text) return clips.reply(m.chat, Func.example(isPrefix, command, 'Meme Kamu'), m)
         ...
         if (m.quoted ? m.quoted.message : m.msg.viewOnce) {
            ...
            if (!mime) return clips.reply(m.chat, Func.texted('bold', `{ SlrmyApi }  Reply photo.`), m)
            if (!/image\/(jpe?g|png|webp)/.test(mime)) return clips.reply(m.chat, Func.texted('bold', `{ SlrmyApi }  Only for photo.`), m)
