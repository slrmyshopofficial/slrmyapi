   ...
   async: async (m, {
      ...
      try {
         ...
         if (m.quoted ? m.quoted.message : m.msg.viewOnce) {
            ...
            if (/video/.test(type)) {
               if (q.seconds > 30) return clips.reply(m.chat, Func.texted('bold', `{ SlrmyApi }   Maximum video duration is 30 seconds.`), m)
            ...
            } else clips.reply(m.chat, `{ SlrmyApi }  To create a watermark on sticker reply media photo or video and use this format *${isPrefix + command} packname | author*`, m)
         }
      } catch (e) {
         console.log(e)
