exports.run = {
   ...
   async: async (m, {
      ...
      try {
         if (m.quoted && (m.quoted.text).match(/gcopt/g) && m.quoted.sender == clips.decodeJid(clips.user.id)) {
            ...
            const groupMetadata = await (await clips.groupMetadata(jid))
            const groupName = groupMetadata.subject
            const adminList = await clips.groupAdmin(jid)
            const admin = adminList.includes((clips.user.id.split`:` [0]) + '@s.whatsapp.net')
            ...
            try {
               pic = await clips.profilePictureUrl(jid, 'image')
            } catch {
            ...
            if (!useOpt) return clips.sendMessageModify(m.chat, steal(Func, data) + '\n\n' + global.footer, m, {
               largeThumb: true,
               thumbnail: await Func.fetchBuffer(pic)
            })
            if (option == 'open') {
               if (!admin) return clips.reply(m.chat, Func.texted('bold', `🚩 Can't open ${groupName} group link because the bot is not an admin.`), m)
               clips.groupSettingUpdate(jid, 'not_announcement').then(() => {
                  clips.reply(jid, Func.texted('bold', `🚩 Group has been opened.`)).then(() => {
                     clips.reply(m.chat, Func.texted('bold', `🚩 Successfully open ${groupName} group.`), m)
                  })
               })
            } else if (option == 'close') {
               if (!admin) return clips.reply(m.chat, Func.texted('bold', `🚩 Can't close ${groupName} group link because the bot is not an admin.`), m)
               clips.groupSettingUpdate(jid, 'announcement').then(() => {
                  clips.reply(jid, Func.texted('bold', `🚩 Group has been closed.`)).then(() => {
                     clips.reply(m.chat, Func.texted('bold', `🚩 Successfully close ${groupName} group.`), m)
                  })
               })
            } else if (option == 'mute') {
               group.mute = true
               clips.reply(m.chat, Func.texted('bold', `🚩 Bot successfully muted in ${groupName} group.`), m)
            } else if (option == 'unmute') {
               group.mute = false
               clips.reply(m.chat, Func.texted('bold', `🚩 Bot successfully unmuted in ${groupName} group.`), m)
            } else if (option == 'link') {
               if (!admin) return clips.reply(m.chat, Func.texted('bold', `🚩 Can't get ${groupName} group link because the bot is not an admin.`), m)
               clips.reply(m.chat, 'https://chat.whatsapp.com/' + (await clips.groupInviteCode(jid)), m)
            } else if (option == 'leave') {
               clips.reply(jid, `🚩 Good Bye! (${global.db.setting.link})`, null, {
                  mentions: groupMetadata.participants.map(v => v.id)
               }).then(() => {
                  clips.groupLeave(jid).then(() => {
                     global.db.groups.find(v => v.jid == jid).expired = 0
                     global.db.groups.find(v => v.jid == jid).stay = false
                     return clips.reply(m.chat, Func.texted('bold', `🚩 Successfully leave from ${groupName} group.`), m)
                  })
               })
            } else if (option == 'reset') {
               global.db.groups.find(v => v.jid == jid).expired = 0
               global.db.groups.find(v => v.jid == jid).stay = false
               clips.reply(m.chat, Func.texted('bold', `🚩 Configuration of bot in the ${groupName} group has been successfully reseted to default.`), m)
            } else if (option == 'forever') {
               group.expired = 0
               group.stay = true
               clips.reply(m.chat, Func.texted('bold', `🚩 Successfully set bot to stay forever in ${groupName} group.`), m)
            } else if (args[1] + ['d'].includes(option)) {
               let now = new Date() * 1
               let day = 86400000 * parseInt(option.replace('d', ''))
               group.expired += (group.expired == 0) ? (now + day) : day
               group.stay = false
               clips.reply(m.chat, Func.texted('bold', `🚩 Bot duration is successfully set to stay for ${option.replace('d', ' day')} in ${groupName} group.`), m)
